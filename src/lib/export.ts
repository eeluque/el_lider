...
export function formatReportPeriodSubtitle(from: string, to: string, subtitlePrefix?: string): string {
  ...
  return from.slice(0, 10) === to.slice(0, 10) ? a : `Periodo: ${a} – ${b}`;
}
  ...
  try {
    ...
    autoTable(doc, {
      ...
      didParseCell: (data) => {
        ...
        // ── Fila completa rojo suave si alguna celda de esa fila tiene "Estado" = "Bajo…" ──
        const estadoVal = String(data.row.cells[headers.indexOf("Estado")]?.raw ?? "");
        if (estadoVal.toLowerCase().includes("bajo")) {
            data.cell.styles.fillColor = [254, 248, 248]; // rgb(254, 248, 248)
            if (colName === "Estado") {
...
export async function exportToExcel(title: string, rows: ExportRow[], options?: ExportFileOptions): Promise<void> {
  try {
    ...
    chunks.forEach((chunk, sheetIdx) => {
        ...
        // ── Fila 3 (r:2): vacía (solo cuando hay ingrediente) ──
        // ── Fila 4 (r:3): etiqueta ingrediente con fondo #753B19 ──
        const hasIngredient = !!options?.ingredientLabel;
        ...
        // ── Headers: r:4 con ingrediente / r:2 sin ingrediente ──
        ...
        // ── Datos: r:5 con ingrediente / r:3 sin ingrediente ──
